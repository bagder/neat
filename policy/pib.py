...
from policy import PropertyArray, PropertyMultiArray, dict_to_properties
...
class PIB(list):
    ...
    def load_policies(self, policy_dir=POLICY_DIR):
        ...
        for filename in os.listdir(policy_dir):
            if filename.endswith(self.file_extension) and not filename.startswith(('.', '#')):
                ...
                p = load_policy_json(policy_dir + filename)
                if p:
                    self.register(p)
    ...
    def lookup(self, input_properties, apply=True):
        """
