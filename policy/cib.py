...
from policy import  NEATRequest, NEATCandidate, NEATProperty, PropertyDict, NEATPropertyError
...
class CIB2(object):
    ...
    def load_cib(self, cib_dir='cib/'):
        ...
        for filename in os.listdir(cib_dir):
            # TODO fix order of loading CIB sources
            if filename.endswith(('.local', '.cib', '.connection')):
                print('loading CIB source %s' % filename)
                p = self.load_json(cib_dir + filename)
                if not p:
                    continue
...
if __name__ == "__main__":
    ...
    cib = CIB2('/Users/bozakz/git/neat/policy/cib/example2/')
